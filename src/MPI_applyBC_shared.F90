...
#include "Solver.h"
        ...
        if(x_id < x_procs - 1) then
                ...
                if(shared_part(myid+1+1) /= MPI_UNDEFINED) then
                        ...
                        call MPI_RECV(local(Nxb+2,:), Nyb+2, MPI_REAL, x_id+1, 1, x_comm, ierr)
        ...
        if(x_id > 0) then
                ...
                if(shared_part(myid+1-1) /= MPI_UNDEFINED) then
                        ...
                        call MPI_RECV(local(1,:), Nyb+2, MPI_REAL, x_id-1, 2, x_comm, ierr)
        ...
        if(y_id < y_procs - 1) then
                ...
                if(shared_part(myid+1+x_procs) /= MPI_UNDEFINED) then
                        ...
                        call MPI_RECV(local(:,Nyb+2), Nxb+2, MPI_REAL, y_id+1, 3, y_comm, ierr)
       ...
       if(y_id > 0) then
                ...
                if(shared_part(myid+1-x_procs) /= MPI_UNDEFINED) then
                        ...
                        call MPI_RECV(local(:,1), Nxb+2, MPI_REAL, y_id-1, 4, y_comm, ierr)
