program MPI_SHMtest
        ...
        integer :: x_comm,y_comm,x_id,y_id,x_procs,y_procs,x_grp,y_grp
        ...
        integer, allocatable,dimension(:) :: world_part,shared_part,x_part,y_part
        ...
        call MPI_COMM_GROUP(x_comm, x_grp, ierr)

        call MPI_COMM_RANK(y_comm,y_id,ierr)
        call MPI_COMM_size(y_comm,y_procs,ierr)
        call MPI_COMM_GROUP(y_comm, y_grp, ierr)

        allocate(x_part(procs))
        allocate(y_part(procs))

        do i=1,procs
                x_part(i) = mod(i-1,nblockx)
        end do
        ...
        call MPI_GROUP_TRANSLATE_RANKS(x_grp,procs,x_part,shared_grp,shared_part,ierr)
